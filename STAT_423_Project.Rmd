...
```{r}
Nresamples <- 5000
n <- length(Skip_class.imputed$Percentage.Skip)

bootmean.skip <- numeric(Nresamples)
bootsd.skip <- numeric(Nresamples)

for(i in 1:Nresamples)
{ bootdata <- sample(Skip_class.imputed$Percentage.Skip, n, replace = T)

  bootmean.skip[i] <- mean(bootdata)
